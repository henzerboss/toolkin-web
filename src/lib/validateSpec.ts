...
export function validateSpec(input:unknown):SpecValidation{
 ...
 checkWiring(spec,errors);if(errors.length>60)errors.length=60;return errors.length?{ok:false,errors}:{ok:true,spec};
}
...
function checkNode(node:UiNode,spec:MiniAppSpec,errors:string[],evaluator:ExpressionEvaluator,path:string,localProps:Set<string>,customStack:string[]):void{
 ...
 if(node.type==='Button'&&Array.isArray(node.onPress)){const paid=node.onPress.some((raw)=>raw&&typeof raw==='object'&&!Array.isArray(raw)&&['llm.ask','image.generate'].includes(String((raw as unknown as Step).action??'')));if(paid&&!(typeof node.disabled==='string'&&/\bllmBusy\b/.test(node.disabled)))errors.push(`${path} (Button): paid AI action must be disabled while llmBusy to prevent duplicate charges`);}
...
export function lintSpec(spec:MiniAppSpec):string[]{const warnings:string[]=[];checkUxV2(spec,warnings);return warnings;}
function checkWiring(spec:MiniAppSpec,errors:string[]):void{
 ...
 for(const step of named('timer.start'))if(step.seconds===undefined)errors.push('timer.start: seconds is missing');
...
function checkSandbox(spec:MiniAppSpec,nodes:UiNode[],errors:string[]):void{
 ...
 if(sandboxes.length&&!spec.capabilities.includes('sandbox'))errors.push('Sandbox: capability "sandbox" is missing');
