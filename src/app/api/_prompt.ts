...
const PLAYBOOK: Record<string, string[]> = {
  ...
  countdown: [
    ...
    '',
    'A date of 0 is 1 January 1970, so every formula built on it produces nonsense until',
    'the person picks a date: "next cycle 29 Jan 1970", "day 20680". Always hide those',
    'blocks until the date exists, and offer the action instead:',
    '  { "type": "EmptyState", "title": "Mark the first day", "hint": "Then the forecast appears",',
    '    "visible": "lastPeriod == 0" }',
    '  { "type": "Stat", "label": "Days left", "value": "{{daysLeft | integer}}",',
    '    "visible": "lastPeriod > 0" }',
    'The same applies to any KeyValue or hint that formats a computed date.',
    '',
