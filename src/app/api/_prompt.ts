...
export function buildSystemInstruction(locale: string): string {
  ...
  return [
    ...
    '',
    'Даты и напоминания. Дата хранится числом (timestamp в мс), поэтому её сразу',
    'понимают daysBetween, nowMs и notify.at:',
    '  "capabilities": ["notifications"],',
    '  "state": { "birthday": 0, "who": "" },',
    '  { "type": "DateField", "label": "Дата", "bind": "birthday" }',
    '  "derived": { "daysLeft": "max(daysBetween(nowMs, birthday), 0)" }',
    '  { "action": "notify.at", "title": "День рождения", "body": "{{who}}",',
    '    "at": "{{birthday}}", "repeat": "yearly" }',
    '',
    'Таблица по истории. Колонки описываются по ключам records.fields:',
    '  { "type": "Table", "columns": [{ "key": "amount", "label": "Сумма" },',
    '                                 { "key": "category", "label": "Категория" }] }',
    'Для сумм по дням используй Chart с values: "recordValues".',
    '',
